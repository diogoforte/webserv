...
static std::vector<ServerStuff> ports;
...
void WebServer::setup_server_sockets()
{
    ...
    for (std::vector<Server>::iterator it = servers_.begin(); it != servers_.end(); ++it)
    {
        ...
        int const flags = fcntl(server_socket, F_GETFL, 0);
        ...
        in_addr_t ip = host == "localhost" ? inet_addr("127.0.0.1") : inet_addr(host.c_str());
        ...
        serverStuff.ip = ip;
        serverStuff.port = it->get_port();
        serverStuff.id = it->getID();

        int hipoteticalReplacingSocket = findPort(serverStuff);
...
void WebServer::accept_connection(Server &server) const
{
    ...
    int const clientSocket = accept(server.get_socket(), reinterpret_cast<sockaddr *>(&address), &address_length);
    ...
    const int option_value = 1;
    if (setsockopt(clientSocket, SOL_SOCKET, SO_REUSEADDR, &option_value, sizeof(option_value)) == -1)
    {
        perror("setsockopt: ");
        log("Failed to set server socket options", warning);
    }
            ...
            for (int i = 0; i < num_events; i++)
            {
                ...
                if (events_[i].events & EPOLLERR || events_[i].events & EPOLLHUP)
                {
                    ...
                    server_it != servers_.end()
                        ? end_connection(*server_it, *client_it)
